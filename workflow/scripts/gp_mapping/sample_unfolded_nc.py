...
if __name__ ==  "__main__":
    ...
    parser.add_argument("-i", "--input", help="Input: nc to gt file", required=True)
    parser.add_argument("-n", "--num", help="Num of genotypes to pick", required=True, type=int)

    parser.add_argument("-o", "--output", help="File output for genotypes",
                        required=True)
    ...
    nc_to_gt = read_nc_to_gt_file(args.input)

    total = sum(len(v) for v in nc_to_gt.values())
    print(f"total genotypes across {len(nc_to_gt)} components: {total}", flush=True)
    ...
    print(f"target={target}, achieved={result['achieved_n']}, "
          ...
          f"n_components_selected={len(result['selected_ids'])}", flush=True)

    with open(args.output, "w") as f:
        for g in result["genotypes"]:
            f.write(g + "\n")
