...
def generate_landscapes(phenotypes, decay="exponential",
...
def write_landscapes_separate(landscapes, output, prefix="landscape",
                               digits=None):
    ...
    ### THIS CODE SHOULD BE FOR MULTIPELE FL BUT I MADE IT SO IT IS ONLY MADE FOR 1
    paths = []
...
def main():
    ...
    ap.add_argument("--prefix", default="landscape",
                     help="Filename prefix for the per-landscape files.", required=False)
    ...
    landscapes = generate_landscapes(
        phenotypes, decay=args.decay,
        corr_length=args.corr_length, mix=args.mix, seed=seed)

    paths = write_landscapes_separate(landscapes, args.output,
                                       prefix=args.prefix)
    print(f"Wrote {len(paths)} landscape files to {args.output}/ "
